...
module Irc2Murmur
  def self.run(config=FileConfig.new)
    ...
    mingle = Mingle.new(HTTPClient.new, config.mingle_protocol, config.mingle_host, config.mingle_port, config.mingle_project, config.mingle_user, config.mingle_password, config.mingle_certs)
